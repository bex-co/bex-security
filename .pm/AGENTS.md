...
Keep status in sync across all three places it lives: the workstream README checkbox, the milestone README `**Status:**` line + task-table `— DONE` markers, and each task's `status:` frontmatter. When scanning the board for open work, skip `done/` directories. If other tooling references a milestone by its pre-move path, that path is _expected_ to disappear on completion — do not recreate it.
