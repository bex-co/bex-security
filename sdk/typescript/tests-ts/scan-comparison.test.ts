...
describe("semantic scan comparison", () => {
  ...
  test.each([
    ...
    "supplies %s authentication to AcpCodex matching",
  ...
  test("does not substitute managed login for an explicitly configured command provider", async () => {
    ...
    // Invalid auth remains native AcpCodex's responsibility, without login fallback.
    await writeFile(
  ...
  test.each(["chatgpt", "api-key"] as const)(
    ...
    async (auth) => {
      ...
      const startThread = spyOn(
        ...
        (options) =>
          codex.startThread(options!) as unknown as ReturnType<
            AcpCodex["startThread"]
          >,
      );
      try {
  ...
  test("does not start AcpCodex when either scan has no findings", async () => {
    const codex: NonNullable<ScanComparisonOptions["codex"]> = {
      startThread() {
  ...
  test.each([
    ...
    "confirms %s without starting AcpCodex",
