...
export class CodexSecurity {
  ...
  async #run(
    ...
    try {
      ...
      const recipe = scanRecipe({
        ...
        target: normalized,
        mode,
        agent: configuredAgent(this.config),
  ...
  #createSessionCodex(
    ...
    const commandAuth =
      configuredAgent(this.config) === "codex" && hasCommandAuth(sessionConfig);
    const agent = configuredAgent(this.config);
  ...
  async #prepareSession(
    ...
    try {
      ...
      const agent = configuredAgent(this.config);
      const claudeProvider = configuredClaudeProvider(this.config);
      const commandAuth = agent === "codex" && hasCommandAuth(requestedConfig);
      const modelProvider =
