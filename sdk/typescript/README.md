...
and `npx @bex-co/bex-security logout`. Login, logout, scans, validation, patching,
and fix verification share a private credential home for stored OpenAI credentials,
including custom providers with `requires_openai_auth = true`:
...
when running a command from a subdirectory. On managed Windows devices, inherited
access for `SYSTEM` and local `Administrators` is preserved while protecting the
home against future changes to its parents. Other users and broad groups are
rejected, and PowerShell Constrained Language Mode is supported. Codex carries
...
```bash
bex-security scan . -c codex-security.yaml --dry-run --json
bex-security scan . -c codex-security.json --model gpt-5.6-terra
bex-security init
bex-security info -c codex-security.yaml --json
```
...
```bash
npx @bex-co/bex-security bulk-scan repositories.csv \
